...
public class NotificationParser {
	...
	static final byte INTERSECTION_UPDATE = 0x2a; // decimal 42
	...
	public static Notification parse(Vehicle vehicle, byte[] bytes) {
		...
		case POSITION_UPDATE: {
			...
			*/
			...
			return new PositionUpdate(vehicle, bytes[2], roadPiece, bytes[3], ! ((bytes[10] & 0x40) == 0x40),
					bytestoInt(bytes[8], bytes[9]));
		}
		...
		case INTERSECTION_UPDATE: {
			...
			*/
			...
			return new IntersectionUpdate(vehicle, intersectionCode, bytes[8] == 1);
		}
	...
	private static int bytestoInt(byte byte0, byte byte1) {
		int asInt = (byte0 & 0xFF) | ((byte1 & 0xFF) << 8);
		return asInt;
	}
