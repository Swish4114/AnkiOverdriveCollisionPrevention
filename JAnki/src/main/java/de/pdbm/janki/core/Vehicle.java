...
import de.pdbm.janki.core.notifications.*;
import tinyb.*;
...
public class Vehicle {
	...
	public void setSpeed(int speed, int acceleration) {
		if (bluetoothDevice.getConnected()) {
			writeCharacteristic.writeValue(Message.speedMessage((short) speed, (short) acceleration));
			this.speed = speed;
		} else {
			System.out.println("not connected");
		}
	...
	private void onValueNotification(byte[] bytes) {
		...
		try {
			...
			} else if (notification instanceof IntersectionUpdate) {
                IntersectionUpdate iu = (IntersectionUpdate) notification;
                for (NotificationListener notificationListener : listeners) {
                    if (notificationListener instanceof IntersectionUpdateListener) {
                        ((IntersectionUpdateListener) notificationListener).onIntersectionUpdate(iu);
                    }
                }
	...
	private static class AnkiBle {
		...
		private static void init() {
			...
			System.out.println("vehicles after discover: " + Vehicle.getVehicles());
					...
					if (vehicle.readCharacteristic == null) {
						...
						if (vehicle.readCharacteristic != null) {
							vehicle.readCharacteristic.enableValueNotifications(bytes -> {
								cachedThreadPool.execute(() -> vehicle.onValueNotification(bytes));
							});
